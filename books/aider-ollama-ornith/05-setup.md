...
Ornith-1.0-9B（Ollama公式ライブラリ配布サイズ5.6GB）を快適に動かすための目安です。

| 項目 | 目安 |
|---|---|
| メモリ（統合メモリ／VRAM） | 16GB以上を推奨 |
| ディスク空き容量 | モデル本体（5.6GB）＋作業用に10GB程度の余裕 |
| チップ | Apple Silicon（M1以降）を推奨 |
| OS | macOS（Windows/Linuxでも動作しますが本書はmacOSで検証） |
| Ollamaバージョン | **v0.30.11以降が必須**（`ornith:9b`の実行要件） |
...
| メモリ | 16GB（統合メモリ） |
| OS | macOS 26.5.1 |
| Ollama | v0.31.2（`ollama --version` で確認） |

> 本シリーズの他記事（study1など）ではOllama v0.30.6時点の検証結果を掲載していますが、`ornith:9b`（公式ライブラリ版）を使うにはv0.30.11以降へのアップデートが必要です。`brew upgrade ollama` で更新できます。
...
Ornith-1.0-9Bは、Ollamaの公式ライブラリ（[ollama.com/library/ornith](https://ollama.com/library/ornith)）に開発元のDeepReinforce AI自身が公開しています。他の公式ライブラリモデル（`gemma4:e2b`など）と同じ感覚で、名前空間なしの `ornith:9b` でそのまま取得できます。

```sh
ollama pull ornith:9b
```

> ⚠️ Ollama Hub上には、有志によるコミュニティ版の再アップロード（例: `maxwellb/Ornith-1.0-9B`）も存在します。中身は基本的に同じモデルですが、量子化方式やパラメータ設定が公式版と異なる場合があるため、特別な理由がなければ公式ライブラリの `ornith:9b` を使うことをおすすめします。
...
```sh
ollama run ornith:9b "こんにちは。あなたは何ができますか？"
```
